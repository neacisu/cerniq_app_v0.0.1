...
| 6 | Înveliș telemetrie | B7 **nu** folosește `withCognitiveSpan`; folosește Prometheus `e4ReconciliationDurationSeconds`, `e4ReconciliationTotal` (B7 ~L22, L56–59, L105–110). Span OTel v2 rămâne țintă nominală. | v2 L6348 — `cognitive.e4.payment.reconcile-auto`. | Nealiniere: span v2 vs instrumentare curentă (metrici, fără span activ pe B7). |
| 7 | Înveliș politică | Idempotență: doar plăți `PENDING` (`loadPendingPayment`); comentariu anti-halucinare B7 ~L10–11. Tier/autonomy Cedar/OPA: neafișat în handler. | v2 L6339–6347 — Tier 2, HITL pentru acțiuni ireversibile. | Politici Cedar/OPA: țintă arhitectură (v2 §2.2), nelegate explicit în B7. |
