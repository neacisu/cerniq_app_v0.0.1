...
| etapa | E3 |
| familie (v2, prima instanță) | `stock` |
| contract_path | `contracts/neurons/E3/stock--sync--erp.md` |
| ADR familie (indicativ) | [stock](../../adr/families/e3/stock.md) |

## Scop în context real

**Scop declarat în v2:** Sincronizare stoc curent din ERP extern la interval definit. **Comportament în repo:** neaudit până la research manual (DOD 0): handler BullMQ/API, payload, teste — vezi `_CONTRACT_SCHEMA.md`. Acest text nu trebuie generat sau extins automat de scripturi; doar de autor după dovezi.

## Surse audit

- v2 §6: `docs/CognitiveBrain/v2_cerniq_cognitive_brain_master_implementation_plan.md` — linia ~5811 (`### NEURON`).
- Schema: [`_CONTRACT_SCHEMA.md`](_CONTRACT_SCHEMA.md).
- Checklist: [`CONTRACT_AUTHORING_CHECKLIST.md`](CONTRACT_AUTHORING_CHECKLIST.md).

## Instanțe v2

### Instanță 1 — `stock` (linia v2 ~5811)

- **Stage:** E3
- **Family:** stock
- **Catalog nodeKey:** e3:stock:sync-erp
- **Neuron type:** ToolNeuron
- **Swimlane:** stock-management
- **Criticality:** HIGH
...
- **Prometheus metrics:** cerniq_neuron_fires_total{neuron_type="ToolNeuron",stage="E3",swimlane="stock-management"}, cerniq_neuron_duration_seconds{neuron_id="e3:stock:sync-erp"}, cerniq_neuron_confidence{neuron_id="e3:stock:sync-erp"}
- **OTel span name:** cognitive.e3.stock.sync-erp

## Tabel self-aware (13 criterii)

| # | Criteriu | În cod (dovadă) | Țintă v2 / research | Limită evidență |
| --- | --- | --- | --- | --- |
| 1 | Identitate canonică | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. Indiciu mecanic (nu substituie citirea codului): registry literal `da`; catalog `n(` `nodeKey`: `e3:stock:sync-erp`. | v2: `stock:sync:erp`; Catalog nodeKey (v2 bloc): `e3:stock:sync-erp` | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
| 2 | Etapă, familie, swimlane | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | Etapă `E3`, familie `stock`, swimlane `stock-management` (v2). | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
| 3 | Rol declarat | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | Funcție cognitivă: Sincronizare stoc curent din ERP extern la interval definit; analogie: Cortex premotor — interfațare cu instrumente externe | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
...
| 6 | Înveliș telemetrie | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | OTel span (v2): `cognitive.e3.stock.sync-erp`; mapare `cognitive.nodeKey` vs `cognitive.neuron.*`: vezi ADR-0003 + `withCognitiveSpan`. | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
