...
| 6 | Înveliș telemetrie | **Lipsă:** fără `withCognitiveSpan` / worker pentru coadă; nu se poate confirma maparea la atribute reale. | OTel span v2: `cognitive.report.conversion.analyze` (L5433). | Doar țintă până la implementare. |
| 7 | Înveliș politică | **Lipsă** metadata job / praguri în cod pentru acest neuron. | Tier 4 (L5424); «No mandatory HITL. Audit log 90 days.» (L5431). | Cedar/OPA:țintă ADR-0007. |
...
| 11 | Micro-OODA | **Lipsă** cod care să materializeze OODA pentru această coadă. | OBSERVE/ORIENT/DECIDE/ACT (v2 L5429). | Neo4j GraphRAG în ORIENT: țintă v2/ADR dacă se extinde. |
...
- **Stare la 2026-04-11:** **doar țintă** — fără dovadă de worker/span pentru `report:conversion:analyze` în repo.
