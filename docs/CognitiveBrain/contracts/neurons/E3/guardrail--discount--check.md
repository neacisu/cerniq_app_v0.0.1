...
**Scop declarat în v2:** Guardrail DETERMINISTIC discount: blocare >50%, escaladare la director >30%. **Comportament în repo:** neaudit până la research manual (DOD 0): handler BullMQ/API, payload, teste — vezi `_CONTRACT_SCHEMA.md`. Acest text nu trebuie generat sau extins automat de scripturi; doar de autor după dovezi.

## Surse audit

- v2 §6: `docs/CognitiveBrain/v2_cerniq_cognitive_brain_master_implementation_plan.md` — linia ~5043 (`### NEURON`).
- Schema: [`_CONTRACT_SCHEMA.md`](_CONTRACT_SCHEMA.md).
- Checklist: [`CONTRACT_AUTHORING_CHECKLIST.md`](CONTRACT_AUTHORING_CHECKLIST.md).

## Instanțe v2

### Instanță 1 — `guardrails` (linia v2 ~5043)

- **Stage:** E3
- **Family:** guardrails
- **Catalog nodeKey:** e3:guardrail:discount-check
- **Neuron type:** GuardrailNeuron
...
- **OODA micro-cycle:** OBSERVE: intercept action payload. ORIENT: run NeMo Guardrails + deterministic checks. DECIDE: allow/block/escalate. ACT: gate downstream execution.
...
- **Prometheus metrics:** cerniq_neuron_fires_total{neuron_type="GuardrailNeuron",stage="E3",swimlane="ai-reasoning"}, cerniq_neuron_duration_seconds{neuron_id="e3:guardrail:discount-check"}, cerniq_neuron_confidence{neuron_id="e3:guardrail:discount-check"}
- **OTel span name:** cognitive.e3.guardrail.discount-check

## Tabel self-aware (13 criterii)

| # | Criteriu | În cod (dovadă) | Țintă v2 / research | Limită evidență |
| --- | --- | --- | --- | --- |
| 1 | Identitate canonică | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. Indiciu mecanic (nu substituie citirea codului): registry literal `da`; catalog `n(` `nodeKey`: `e3:guardrail:discount-check`. | v2: `guardrail:discount:check`; Catalog nodeKey (v2 bloc): `e3:guardrail:discount-check` | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
| 2 | Etapă, familie, swimlane | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | Etapă `E3`, familie `guardrails`, swimlane `ai-reasoning` (v2). | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
| 3 | Rol declarat | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | Funcție cognitivă: Guardrail DETERMINISTIC discount: blocare >50%, escaladare la director >30%; analogie: Cortex cingulat — gardian al limitelor și siguranței | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
...
| 6 | Înveliș telemetrie | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | OTel span (v2): `cognitive.e3.guardrail.discount-check`; mapare `cognitive.nodeKey` vs `cognitive.neuron.*`: vezi ADR-0003 + `withCognitiveSpan`. | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
...
| 11 | Micro-OODA | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | OBSERVE: intercept action payload. ORIENT: run NeMo Guardrails + deterministic checks. DECIDE: allow/block/escalate. ACT: gate downstream execution. | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
