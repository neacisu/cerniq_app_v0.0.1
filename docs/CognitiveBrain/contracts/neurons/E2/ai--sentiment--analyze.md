...
| 9 | Guardrails | **Zod** `sentimentAnalysisSchema`; `generateValidatedJsonWithRetries` max 3 încercări (`ai-sentiment.ts` L373–397). **Fără** NeMo în fișier. | v2 + ADR-0007. | NeMo: destinație documentată. |
...
| 11 | Micro-OODA | Observe (mesaj + cache), Orient (LLM JSON), Decide (schema + praguri score/intent), Act (DB update + enqueue response sau review). **Fără** GraphRAG. | v2 OODA; GraphRAG — destinație ADR-0005. | — |
