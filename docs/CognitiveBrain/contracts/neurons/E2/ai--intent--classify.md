...
**Scop declarat în v2:** Clasificare intenție mesaj primit (interesat/refuz/…). **Comportament în repo:** neaudit până la research manual (DOD 0): handler BullMQ/API, payload, teste — vezi `_CONTRACT_SCHEMA.md`. Acest text nu trebuie generat sau extins automat de scripturi; doar de autor după dovezi.

## Surse audit

- v2 §6: `docs/CognitiveBrain/v2_cerniq_cognitive_brain_master_implementation_plan.md` — linia ~3072 (`### NEURON`).
- Schema: [`_CONTRACT_SCHEMA.md`](_CONTRACT_SCHEMA.md).
- Checklist: [`CONTRACT_AUTHORING_CHECKLIST.md`](CONTRACT_AUTHORING_CHECKLIST.md).

## Instanțe v2

### Instanță 1 — `ai-analysis` (linia v2 ~3072)

- **Stage:** E2
- **Family:** ai-analysis
- **Catalog nodeKey:** e2:ai:intent-classify
- **Neuron type:** DeliberativeNeuron
...
### Instanță 2 — v2 «duplicat #2» — `ai-core` (linia v2 ~4552)
...
- **OODA micro-cycle:** OBSERVE: load context window. ORIENT: LLM reasoning with structured output. DECIDE: confidence gating (>0.80 pass, <0.80 escalate). ACT: emit decision + update episodic memory.
...
- **Prometheus metrics:** cerniq_neuron_fires_total{neuron_type="DeliberativeNeuron",stage="E2",swimlane="ai-analysis"}, cerniq_neuron_duration_seconds{neuron_id="e2:ai:intent-classify"}, cerniq_neuron_confidence{neuron_id="e2:ai:intent-classify"}
- **OTel span name:** cognitive.e2.ai.intent-classify

## Tabel self-aware (13 criterii)

| # | Criteriu | În cod (dovadă) | Țintă v2 / research | Limită evidență |
| --- | --- | --- | --- | --- |
| 1 | Identitate canonică | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. Indiciu mecanic (nu substituie citirea codului): registry literal `nu`; catalog `n(` `nodeKey`: `e2:ai:intent-classify`. | v2: `ai:intent:classify`; Catalog nodeKey (v2 bloc): `e2:ai:intent-classify` | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
...
| 3 | Rol declarat | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | Funcție cognitivă: Clasificare intenție mesaj primit (interesat/refuz/…); analogie: Cortex prefrontal — deliberare și raționament profund | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
...
| 6 | Înveliș telemetrie | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | OTel span (v2): `cognitive.e2.ai.intent-classify`; mapare `cognitive.nodeKey` vs `cognitive.neuron.*`: vezi ADR-0003 + `withCognitiveSpan`. | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
...
| 11 | Micro-OODA | **TODO manual (DOD 0–4):** parcurgeți v2 → catalog → registry → handler/payload → teste; notați fișier + simbol sau «lipsă la audit». Interzis completarea din șabloane familie sau din script. | OBSERVE: load context window. ORIENT: LLM reasoning with structured output. DECIDE: confidence gating (>0.80 pass, <0.80 escalate). ACT: emit decision + update episodic memory. | v2 §2.4 — completare «În cod» doar după citire cod/teste; fără presupuneri între neuroni. |
