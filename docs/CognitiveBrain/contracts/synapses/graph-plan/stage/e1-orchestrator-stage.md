# Sinapsă `e1-orchestrator-stage`

## Identitate

| Câmp | Valoare |
| --- | --- |
| Identificator sinapsă | `e1-orchestrator-stage` |
| Cale contract | `docs/CognitiveBrain/contracts/synapses/graph-plan/stage/e1-orchestrator-stage.md` |
| Areal sinaptic | `graph-plan` |
| Traseu sinaptic | `e1-orchestrator-stage` |
...
| --- | --- | --- |
| Sursă | `e1-orchestrator` | Etichetă **swimlane / subgraf** în planificare; **nu** este prin ea însăși un `v2_queue` unic. **Necesită reconciliere graf ↔** [`NEURON_MATRIX.csv`](../../../../NEURON_MATRIX.csv) **/ contracte neuroni / registry** pentru fiecare neuron concret din swimlane. |
...
- **Planificare:** v2 §7 — `e1-orchestrator` → `e1` (muchie `e1-orchestrator-stage`).
...
`e1-orchestrator` marchează în graf o swimlane de **orchestrare** a subfluxurilor din **E1**. Atenție: „orchestrator” aici este **nod de planificare**, nu echivalent automat cu un singur worker sau o coadă din `queue-registry.ts` fără mapare dovedită.

## Sursă canonică

- [`../../../../v2_cerniq_cognitive_brain_master_implementation_plan.md`](../../../../v2_cerniq_cognitive_brain_master_implementation_plan.md) — §7, bloc `SYNAPSE \`e1-orchestrator-stage\``.
