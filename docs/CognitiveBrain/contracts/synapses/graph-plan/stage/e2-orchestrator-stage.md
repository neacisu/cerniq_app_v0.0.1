# Sinapsă `e2-orchestrator-stage`

## Identitate

| Câmp | Valoare |
| --- | --- |
| Identificator sinapsă | `e2-orchestrator-stage` |
| Cale contract | `docs/CognitiveBrain/contracts/synapses/graph-plan/stage/e2-orchestrator-stage.md` |
| Areal sinaptic | `graph-plan` |
| Traseu sinaptic | `e2-orchestrator-stage` |
...
| --- | --- | --- |
| Sursă | `e2-orchestrator` | Etichetă **swimlane / subgraf** în planificare; **nu** este prin ea însăși un `v2_queue` unic. **Necesită reconciliere graf ↔** [`NEURON_MATRIX.csv`](../../../../NEURON_MATRIX.csv) **/ contracte neuroni / registry** pentru fiecare neuron concret din swimlane. |
...
- **Planificare:** v2 §7 — `e2-orchestrator` → `e2` (muchie `e2-orchestrator-stage`).
...
`e2-orchestrator` marchează **orchestrarea** subfluxurilor din **E2**. Analog cu `e1-orchestrator`, eticheta este **de planificare**; **nu se echivalează** automat cu un proces unic în `queue-registry.ts` fără mapare explicită.

## Sursă canonică

- [`../../../../v2_cerniq_cognitive_brain_master_implementation_plan.md`](../../../../v2_cerniq_cognitive_brain_master_implementation_plan.md) — §7, bloc `SYNAPSE \`e2-orchestrator-stage\``.
