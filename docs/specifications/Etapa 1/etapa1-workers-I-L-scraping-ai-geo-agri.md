...
async function verifyCompanyWebsite(
  url: string,
  denumire: string,
  cui: string,
): Promise<{
  ...
  processor: async (job, logger) => {
    ...
    try {
      ...
      if (contactData.emails?.length > 0) {
        const mainEmail =
          contactData.emails.find(
            (e: string) => !e.startsWith("info@") && !e.startsWith("contact@"),
  ...
  "not_found": ["field1", "field2"]
      ...
      const response = await xai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "Ești un asistent de cercetare business cu acces la web.",
          },
          { role: "user", content: prompt },
        ],
        temperature: 0.3,
