...
# 2. WORKER #9: q:wa:phone\_{XX} (Initial Send)
...
# 3. WORKER #10: q:wa:phone\_{XX}:followup
...
for (let i = 1; i <= 20; i++) {
  ...
  PHONE_QUEUES.push(
    new Queue(`q:wa:phone_${suffix}:followup`, {
      connection: REDIS_CONNECTION,
    }),
  );
}
