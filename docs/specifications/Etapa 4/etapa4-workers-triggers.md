...
const ETAPA4_CRON_JOBS: CronJobConfig[] = [
  // Daily jobs
  {
    name: "credit:refresh:all",
    pattern: "0 3 * * *",
    queue: "pipeline:credit:refresh-all",
  },
  {
    name: "overdue:detect",
    pattern: "0 9 * * *",
    queue: "payment:overdue:detect",
  },
  {
    name: "compliance:check",
    pattern: "0 6 * * *",
    queue: "audit:compliance:check",
  },
  {
    name: "daily:summary",
    pattern: "0 18 * * 1-5",
    queue: "alert:internal:daily-summary",
  },
  {
    name: "contract:expire",
    pattern: "0 1 * * *",
    queue: "pipeline:contract:expire",
  },
  ...
  { name: "stock:sync", pattern: "*/15 * * * *", queue: "stock:sync:oblio" },
  {
    ...
    pattern: "*/15 * * * *",
    queue: "pipeline:reservation:expire",
  },
  {
    name: "balance:sync",
    pattern: "*/30 * * * *",
    queue: "revolut:balance:sync",
  },
...
const ORDER_STATE_TRIGGERS: Record<string, TriggerConfig[]> = {
  ...
  CONTRACT_SIGNED: [{ queue: "sameday:awb:create", condition: "always" }],
  PICKED_UP: [{ queue: "alert:client:shipped", condition: "always" }],
  DELIVERED: [
...
const PAYMENT_EVENT_TRIGGERS = {
  ...
  PAYMENT_RECONCILED: [{ queue: "payment:balance:update" }],
  PAYMENT_OVERDUE: [
