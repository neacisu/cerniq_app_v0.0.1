...
```json
{
  ...
  "panels": [
    ...
    },
    {
      ...
      "targets": [
        {
          "expr": "increase(cerniq_etapa4_shipments_created_total{status=\"DELIVERED\"}[24h])"
        }
      ]
    },
    {
    ...
    },
    {
      ...
      "targets": [
        {
          "expr": "sum(rate(cerniq_etapa4_payments_received_total{reconciliation_status=\"MATCHED_EXACT\"}[1h])) / sum(rate(cerniq_etapa4_payments_received_total[1h]))"
        }
      ]
    }
  ]
  ...
  "panels": [
    {
      ...
      "targets": [
        {
          "expr": "rate(cerniq_etapa4_worker_jobs_processed_total[5m]) by (queue)"
        }
      ]
    },
    {
      ...
      "targets": [
        {
          "expr": "histogram_quantile(0.95, rate(cerniq_etapa4_worker_job_duration_seconds_bucket[5m])) by (queue)"
        }
      ]
    },
    {
      ...
      "targets": [
        {
          "expr": "sum(increase(cerniq_etapa4_worker_jobs_processed_total{status=\"error\"}[1h]))"
        }
      ]
    },
    {
