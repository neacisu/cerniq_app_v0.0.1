...
export async function auditComplianceCheckProcessor(
  ...
  if (oldData.length > 0) {
    issues.push({
      type: "GDPR",
      severity: "HIGH",
      message: `${oldData.length} records exceed 7-year retention`,
    });
  }
...
export async function auditDataAnonymizeProcessor(
  ...
  await logAudit("DATA_ANONYMIZED", "system", "etapa4", correlationId, {
    anonymizedCount,
  });
