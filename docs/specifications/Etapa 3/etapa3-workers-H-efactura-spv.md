...
// Status check queue
const statusCheckQueue = new Queue("efactura:status:check", {
  connection: redis,
});
  ...
  async (job: Job<EfacturaStatusCheckJobData>): Promise<EfacturaStatusCheckResult> => {
    ...
    switch (statusResponse.status) {
      ...
      case EfacturaStatus.EROARE_VALIDARE:
        result = await handleRejectedStatus(data, submission, statusResponse);
        break;

      case EfacturaStatus.IN_PRELUCRARE:
      default:
        result = await handleProcessingStatus(data, submission, statusResponse);
        break;
    }
...
async function handleRejectedStatus(
  ...
  const validationErrors =
    statusResponse.erori_validare?.map((e) => ({
      code: e.cod,
      message: e.mesaj,
      location: e.locatie,
    })) || [];
...
export const EFACTURA_REQUIREMENTS = {
  ...
  vatRates: {
    ...
    zero: 0.0, // 0% - exporturi, livrări intracomunitare
  },
} as const;
...
export class UblInvoiceGenerator {
  ...
  private addHeaderElements(doc: any, invoice: UblInvoiceDocument): void {
    ...
    if (invoice.invoicePeriod) {
      doc
        ...
        .ele("cbc:StartDate")
        .txt(invoice.invoicePeriod.startDate)
        .up()
        .ele("cbc:EndDate")
  ...
  private addSupplierParty(doc: any, supplier: UblParty): void {
    ...
    if (supplier.partyIdentification) {
      party
        ...
        .ele("cbc:ID")
        .txt(supplier.partyIdentification)
        .up()
        .up();
    }
    ...
    legal
      .ele("cbc:CompanyID", { schemeID: supplier.companyIdScheme || "RO" })
  ...
  private addCustomerParty(doc: any, customer: UblParty): void {
    ...
    if (customer.partyIdentification) {
      party
        .ele("cac:PartyIdentification")
        .ele("cbc:ID")
        .txt(customer.partyIdentification)
        .up()
        .up();
    }
    ...
    legal
      .ele("cbc:CompanyID", { schemeID: customer.companyIdScheme || "RO" })
  ...
  private addMonetaryTotal(doc: any, total: UblMonetaryTotal): void {
    ...
    lmt
      .ele("cbc:LineExtensionAmount", { currencyID: "RON" })
      .txt(this.formatAmount(total.lineExtensionAmount))
      .up();

    lmt
      .ele("cbc:TaxExclusiveAmount", { currencyID: "RON" })
      .txt(this.formatAmount(total.taxExclusiveAmount))
      .up();

    lmt
      .ele("cbc:TaxInclusiveAmount", { currencyID: "RON" })
    ...
    if (total.chargeTotalAmount !== undefined && total.chargeTotalAmount > 0) {
      lmt
        .ele("cbc:ChargeTotalAmount", { currencyID: "RON" })
    ...
    if (total.prepaidAmount !== undefined && total.prepaidAmount > 0) {
      lmt
        .ele("cbc:PrepaidAmount", { currencyID: "RON" })
        .txt(this.formatAmount(total.prepaidAmount))
        .up();
    }

    lmt
      .ele("cbc:PayableAmount", { currencyID: "RON" })
  ...
  private addInvoiceLine(doc: any, line: UblInvoiceLine): void {
    ...
    if (line.item.standardItemId) {
      item
        ...
        .ele("cbc:ID", { schemeID: "0160" })
        .txt(line.item.standardItemId)
        .up()
        .up();
    }
    ...
    taxCat
      .ele("cac:TaxScheme")
      .ele("cbc:ID")
      .txt(line.item.classifiedTaxCategory.taxScheme)
      .up()
      .up();
    taxCat.up();
    ...
    price
      .ele("cbc:PriceAmount", { currencyID: "RON" })
 ...
 */
export const EFACTURA_ERROR_CLASSIFICATION: Record<
  EfacturaErrorType,
  {
    retryable: boolean;
    maxRetries: number;
...
export function classifyEfacturaError(error: Error | AxiosError | any): EfacturaErrorType {
  ...
  if (error.response) {
    ...
    if (status >= 500) {
      ...
      if (
        data?.message?.includes("sistem indisponibil") ||
        ...
        "fieldConfig": {
          "defaults": {
            "mappings": [
              ...
              {
                "type": "value",
                "options": { "1": { "text": "Open/HalfOpen", "color": "red" } }
              }
            ]
          }
        }
...
export async function checkEfacturaQueueHealth(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const statusCheckHealth = await checkQueueHealth(
      efacturaStatusCheckQueue,
      "efactura:status:check",
      {
        maxWaiting: 500,
        maxFailed: 50,
    ...
    const deadlineHealth = await checkQueueHealth(
      efacturaDeadlineMonitorQueue,
      "efactura:deadline:monitor",
      {
        maxWaiting: 10,
        maxFailed: 5,
