...
async function getActivePromotion(shopId: string, sku: string, productId: string): Promise<number> {
  ...
  const promo = await db.query.price_rules.findFirst({
    where: (pr, { and, eq, lte: leq, gte: geq }) =>
      and(
        eq(pr.shop_id, shopId),
        or(eq(pr.sku, sku), eq(pr.product_id, productId)),
...
export async function discountCalculateProcessor(
  ...
  const effectiveDiscount =
    requestedDiscount !== undefined ? requestedDiscount : recommendedDiscount;
...
function extractPriceMentions(response: string, logger: Logger): PriceMention[] {
  ...
  const skuPricePattern =
    /([A-Z]{2,6}[-_]?\d{2,6})\s*(?:[-:@]|la|costă?|preț(?:ul)?)\s*(\d+(?:[.,]\d{1,2})?)\s*(?:lei|RON)/gi;
...
export async function resolveApplicableRules(
  ...
  const rules = await db.query.price_rules.findMany({
    where: (pr, { and, eq, or, lte, gte, isNull }) =>
      and(
        eq(pr.shop_id, shopId),
        eq(pr.is_active, true),
        or(isNull(pr.start_date), lte(pr.start_date, now)),
        or(isNull(pr.end_date), gte(pr.end_date, now)),
        or(isNull(pr.sku), eq(pr.sku, sku)),
        or(isNull(pr.category_id), eq(pr.category_id, categoryId)),
      ),
...
describe("Discount Calculate Worker", () => {
  ...
  describe("Margin preservation", () => {
    it("caps discount to preserve minimum margin", async () => {
      const result = await discountCalculateProcessor(
        createMockJob({
          sku: "TEST-001",
          quantity: 1,
...
_End of Etapa 3 - Workers Categoria E: Pricing & Discount (Guardrails)_
