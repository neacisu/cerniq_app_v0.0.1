...
async function fetchOfferData(
  ...
  const [offer] = await db
    ...
    .where(and(eq(offers.id, offerId), eq(offers.tenantId, tenantId)))
    .limit(1);
...
async function sendViaSes(
  ...
  const command = new SendEmailCommand({
    ...
    Destination: {
      ...
      CcAddresses: preparedEmail.cc
        ? Array.isArray(preparedEmail.cc)
          ? preparedEmail.cc
          : [preparedEmail.cc]
        : undefined,
      BccAddresses: preparedEmail.bcc
        ? Array.isArray(preparedEmail.bcc)
          ? preparedEmail.bcc
          : [preparedEmail.bcc]
        : undefined,
    },
...
async function sendViaSendGrid(
  ...
  const msg: sgMail.MailDataRequired = {
    ...
    cc: preparedEmail.cc
      ? Array.isArray(preparedEmail.cc)
        ? preparedEmail.cc.map((e) => ({ email: e }))
        : [{ email: preparedEmail.cc }]
      : undefined,
    bcc: preparedEmail.bcc
      ? Array.isArray(preparedEmail.bcc)
        ? preparedEmail.bcc.map((e) => ({ email: e }))
        : [{ email: preparedEmail.bcc }]
      : undefined,
    from: {
...
async function buildRawEmail(preparedEmail: PreparedEmail): Promise<string> {
  ...
  if (preparedEmail.htmlBody) {
    ...
    rawEmail += `\r\n`;
    rawEmail +=
      Buffer.from(preparedEmail.htmlBody)
        .toString("base64")
        .match(/.{1,76}/g)
        ?.join("\r\n") || "";
    rawEmail += `\r\n`;
...
async function processEmailEvent(event: EmailEventData): Promise<void> {
  ...
  if (!emailLog) {
    logger.warn("Email log not found for event", {
      messageId: event.messageId,
    });
    return;
  }
...
emailQueueEvents.on("failed", ({ jobId, failedReason }) => {
  ...
  documentMetrics.emailErrors.inc({
    error_type: categorizeEmailError(failedReason),
  });
});
...
whatsappQueueEvents.on("failed", ({ jobId, failedReason }) => {
  ...
  documentMetrics.whatsappErrors.inc({
    error_type: categorizeWhatsAppError(failedReason),
  });
});
...
export function classifyDocumentError(
  ...
  if (
    ...
    (message.includes("crash") || message.includes("disconnect"))
  ) {
...
export async function handleDocumentRetry<T extends { tenantId: string }>(
  ...
  documentMetrics.retriesScheduled.inc({
    category,
    error_type: docError.errorType,
  });
...
>(
  ...
  breaker.on("reject", () => {
    logger.debug("Document circuit breaker rejected request", {
      category,
      name,
    });
        ...
        "fieldConfig": {
          "defaults": {
            "mappings": [
              {
                "type": "value",
                "options": { "0": { "text": "OPEN", "color": "red" } }
              },
              {
                "type": "value",
                "options": { "0.5": { "text": "HALF-OPEN", "color": "yellow" } }
              },
              {
                "type": "value",
                "options": { "1": { "text": "CLOSED", "color": "green" } }
              }
            ]
          }
        }
...
} from "@/workers/etapa3/document-generation/email/validator";
...
} from "@/workers/etapa3/document-generation/whatsapp/validator";
...
import {
  classifyDocumentError,
  DocumentErrorType,
} from "@/workers/etapa3/document-generation/errors/classification";
...
test.describe("Document Generation E2E", () => {
  test.describe("PDF Generation", () => {
    test("should generate invoice PDF from negotiation", async ({ page }) => {
      ...
      // Wait for success
      await expect(page.getByText("Factură generată cu succes")).toBeVisible({
        timeout: 30000,
      });
  ...
  test.describe("Email Delivery", () => {
    test("should send offer email from negotiation", async ({ page }) => {
      ...
      // Wait for success
      await expect(page.getByText("Email trimis cu succes")).toBeVisible({
        timeout: 15000,
      });
    });
  ...
  test.describe("WhatsApp Messaging", () => {
    test("should send WhatsApp message from contact", async ({ page }) => {
      ...
      await expect(page.getByText("Aceasta este un test E2E")).toBeVisible({
        timeout: 10000,
      });
    ...
    test("should send template message", async ({ page }) => {
      ...
      // Verify success
      await expect(page.getByText("Șablon trimis cu succes")).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe("Document HITL Resolution", () => {
    test("should resolve document generation error via HITL", async ({
      ...
      await expect(page.getByText("Task rezolvat")).toBeVisible({
        timeout: 15000,
      });
    });
  });
...
class DocumentEventBus extends EventEmitter {
  async emit(
    ...
    payload: Omit<DocumentEvent, "type" | "timestamp">,
  ): Promise<boolean> {
  ...
  const [task] = await db
    ...
    .values({
      ...
      payload: {
        ...
        ...(data.relatedNegotiationId && {
          negotiationId: data.relatedNegotiationId,
        }),
