...
export function propagateTrace(parentSpan: Span, childJobData: any): any {
  const context = trace.setSpan(opentelemetry.context.active(), parentSpan);
  ...
  "dashboard": {
    ...
    "panels": [
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "sum(nurturing_referrals_created_total{tenant_id=\"$tenant\"})",
            "legendFormat": "Created"
          },
          {
            "expr": "sum(nurturing_referrals_consent_given{tenant_id=\"$tenant\"})",
            "legendFormat": "Consent"
          },
          {
            "expr": "sum(nurturing_referrals_converted{tenant_id=\"$tenant\"})",
