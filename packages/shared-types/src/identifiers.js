...
function normalizeIdentifierWhitespace(input) {
    return input.trim().toUpperCase().replaceAll(WHITESPACE_RE, "");
}
export function sanitizeCui(input) {
...
export function isValidCUI(input) {
    ...
    if (clean.length < 2 || clean.length > 10 || !/^\d+$/.test(clean))
        return false;
...
export function computeNrRegComCheckDigit(base) {
...
export function convertOldNrRegComToCanonical(type, county, order, year) {
 ...
 */
export function sanitizeNrRegCom(input) {
 ...
 */
export function normalizeNrRegCom(input) {
    const raw = normalizeIdentifierWhitespace(input);
    if (!raw)
        return null;
    const newMatch = NEW_NR_REG_COM_RE.exec(raw);
...
export function isValidNrRegCom(input) {
