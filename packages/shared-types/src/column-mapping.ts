...
export type ColumnMappingEntry = {
  /** Canonical DB / domain field name. */
  key: string;
  /** Human-readable label shown in the mapping UI. */
  label: string;
  /** All known aliases (lowercase-normalizable) that map to this field. */
  aliases: string[];
};

export const COLUMN_MAPPING_DEFINITIONS: readonly ColumnMappingEntry[] = [
  // --- Identificatori ---
  {
    key: "companyName",
 ...
 */
export function buildColumnAliasToTargetMap(
  normalizeFn: (value: string) => string,
): Map<string, string> {
  return new Map(
    COLUMN_MAPPING_DEFINITIONS.flatMap((entry) =>
      entry.aliases.map((alias) => [normalizeFn(alias), entry.key] as const),
    ),
  );
}
