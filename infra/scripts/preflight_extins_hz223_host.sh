...
sudo iptables -S FORWARD | egrep '10\\.0\\.1\\.(108|109|110)/32' | sed -n '1,260p' || true
echo

echo "# iptables FORWARD excerpts (traffic to orchestrator/redis/openbao)"
sudo iptables -S FORWARD | egrep '10\\.0\\.0\\.2/32|77\\.42\\.76\\.185' | sed -n '1,220p' || true
echo
...
sudo iptables -S PVEFW-FORWARD 2>/dev/null | egrep '10\\.0\\.1\\.(108|109|110)/32|10\\.0\\.0\\.2/32|77\\.42\\.76\\.185' | sed -n '1,220p' || true
echo
