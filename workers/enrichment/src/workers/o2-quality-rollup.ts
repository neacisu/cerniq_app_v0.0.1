...
const svcLog = createServiceLogger("o2-quality-rollup", { etapa: "e1" });
  ...
  return withCognitiveSpan(
    ...
    async (_span) => {
      ...
      try {
        ...
        log.step("done", "Quality rollup", {
          latencyMs: Date.now() - startedAt,
          confidenceScore: total / 100,
          pipelineContext: {
            companyId: job.data.companyId,
        ...
        log.error(
          "fatal",
          `Quality rollup eșuat: ${error instanceof Error ? error.message : String(error)}`,
          {
            ...enrichError(error, {
              tenantId: job.data.tenantId,
              entityType: "company",
