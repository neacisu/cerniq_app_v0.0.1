...
const svcLog = createServiceLogger("p3-pipeline-monitor", { etapa: "e1" });
  ...
  return withCognitiveSpan(
    ...
    async (_span) => {
      ...
      const log = createJobLogger({
        ...
        workerName: "P3:pipeline-monitor",
        jobId: String(job.id ?? ""),
        startedAt,
        etapa: "e1",
      ...
      try {
        ...
        svcLog.info({ tenantId: job.data.tenantId }, "P3 pipeline monitor");
        ...
        log.error(
          "fatal",
          `Pipeline monitor eșuat: ${error instanceof Error ? error.message : String(error)}`,
          {
            ...enrichError(error, {
              tenantId: job.data.tenantId,
              entityType: "tenant",
