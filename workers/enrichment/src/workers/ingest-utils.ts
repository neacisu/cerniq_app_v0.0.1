...
async function handleIdentityConflictRow(
  ...
  await getE1IngestWorkerHooks().createHitlApprovalTask({
...
export async function insertBronzeRows(
  ...
  for (const [index, row] of rows.entries()) {
    ...
    payload.push({
      ...
      extractedCuiRaw,
      extractedCui: extractedCuiRaw
        ? getE1IngestWorkerHooks().sanitizeCui(extractedCuiRaw) || null
        : null,
