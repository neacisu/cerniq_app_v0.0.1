...
import { createQueue, QUEUES, DEFAULT_JOB_OPTIONS } from "@cerniq/worker-shared";
...
> = async (job) => {
  ...
  // 30% - 50%: HITL director + CONSENSUS VOTING STUB
  console.info(
    `[E29] CONSENSUS_VOTING_PENDING: reasoningClient + Grok-4 + GPT-4o pentru discount ${requestedDiscountPct}%`,
  );
  const hitlQueue = createQueue(QUEUES.HITL_ESCALATION);
  await hitlQueue.add(
    "hitl:escalate",
